...
- Added an optional OpenAI Responses API provider with strict Structured Outputs and synthetic-data preflight.
- Added a 48-case repo-native benchmark with separate baseline-contract and natural-prose live-provider suites.
- Added canonical proposal/receipt provenance, a versioned OpenAI fixture-hash registry, atomic review-to-manifest materialization and launch-validator enforcement.
- Centralized review transitions so blocked rows cannot be approved through SQLite.
- Made generated artifacts reproducible with `SOURCE_DATE_EPOCH`.
- Replaced signature-only fixture bytes with decodable JPEG and MP4 media.
- Added installable packaging, Ruff, mypy trust-boundary checks, branch coverage and a Python-version CI matrix.
- Curated the public documentation around product, architecture, AI, QA, platform and security evidence.

## 1.0.0 — 2026-07-12

- Published the offline 100-row Creative Launch Workspace, static demo, review state, SQLite proof store, browser QA and non-executable platform preview.
