...
<body>
  ...
  <script>
    (()=>{
      ...
      if(title&&!reduced.matches&&!window.matchMedia('(max-width: 360px)').matches){const label=title.textContent;title.setAttribute('aria-label',label);title.innerHTML=label.split(' ').map((word)=>`<span class="wchar-word" aria-hidden="true">${[...word].map((char)=>`<span class="wchar">${char}</span>`).join('')}</span>`).join(' ');const chars=[...title.querySelectorAll('.wchar')];let point=null;let raf=0;title.addEventListener('pointermove',(event)=>{point={x:event.clientX,y:event.clientY};if(raf)return;raf=requestAnimationFrame(()=>{chars.forEach((char)=>{const rect=char.getBoundingClientRect();const d=Math.hypot(point.x-(rect.left+rect.width/2),point.y-(rect.top+rect.height/2));const weight=d<200?600+(1-d/200)*300:600;char.style.fontVariationSettings=`'wght' ${Math.round(weight)}`});raf=0})});title.addEventListener('pointerleave',()=>chars.forEach((char)=>char.style.fontVariationSettings="'wght' 600"))}
    })();
  </script>
</body>
</html>
