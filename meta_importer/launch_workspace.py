...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .brand-mark::before { left: 2px; top: 2px; border-left: 2px solid; border-top: 2px solid; }
    .brand-mark::after { right: 2px; bottom: 2px; border-right: 2px solid; border-bottom: 2px solid; }
    .brand-mark span {
      position: absolute;
      left: 12px;
      top: 12px;
    ...
    .button-primary:hover { border-color: var(--action-pressed); background: var(--action-pressed); }
    .bulkbar {
      min-height: 48px;
      display: flex;
      align-items: center;
    ...
    .guided-actions button { min-height: 44px; }
    .guided-actions .button-primary { flex: 1 1 180px; }
    .guided-boundary { margin: 16px 0 0; color: var(--muted); font-size: 11px; }
    .guided-proof {
      display: grid;
      grid-template-columns: 1fr 1fr;
      margin: 18px 0 0;
    ...
    .guided-proof div:nth-last-child(-n+2) { border-bottom: 0; }
    ...
    .tone-1 { background: var(--mint); }
    .tone-2 { background: var(--paper); }
    .tone-3 { color: #fff; background: var(--forest); }
    ...
    .proof-visual.format-video, .proof-visual.format-story { background: var(--forest); }
    .proof-visual.format-video i, .proof-visual.format-story i { width: 31%; right: 18%; background: var(--mint); transform: none; }
    ...
    .proof-visual.format-carousel { background: var(--mint); }
    ...
    .proof-visual.format-collection b { width: 38%; height: 38%; right: 8%; bottom: 9%; background: var(--mint); transform: none; }
    ...
    <dialog class="guided-dialog" id="guided-dialog" aria-labelledby="guided-title">
      ...
      <div class="guided-body">
        ...
        <div class="guided-actions" id="guided-step-two" hidden>
          <button class="button-primary" id="guided-confirm" type="button">Confirm reuse</button>
          <button id="guided-fix-action" type="button">Needs fix</button>
          <button class="danger" id="guided-block" type="button">Block launch</button>
        </div>
        <div id="guided-step-three" hidden>
          <dl class="guided-proof">
            ...
            <div><dt>External write</dt><dd id="guided-result-write">not attempted · mutation_allowed:false</dd></div>
          </dl>
          <p class="guided-copy" id="guided-result-copy"></p>
          <div class="guided-actions"><button class="button-primary" id="guided-explore" type="button">Explore the full queue</button><button id="guided-replay" type="button">Review another case</button></div>
        </div>
        <p class="guided-boundary">Synthetic fixture · browser-local state · no Meta API, upload, publish or spend change</p>
      </div>
    </dialog>
  ...
  <script>
    ...
    function renderGuidedStep() {
      ...
      if (guidedStep === 1) {
        ...
        guidedCopy.textContent = "Confirm intentional reuse, request a fix or block the launch. The choice changes browser-local review state only.";
      } else {
        ...
        guidedCopy.textContent = row
          ? "The decision and audit event are local, inspectable and explicitly unable to write to an external platform."
    ...
    function completeGuidedDemo(row, event) {
      const ambiguousCount = workspaceData.review_statuses.filter((item) => item.batch_state === "needs_review").length;
      guidedStep = 3;
      ...
      document.getElementById("guided-result-copy").textContent = "You just reviewed one of " + ambiguousCount + " ambiguous cases. No external system was changed.";
      renderGuidedStep();
    }
    ...
    function updateReview(status, decision) {
      ...
      if (status === "confirmed_ready" && row.batch_state === "blocked") {
        statusLine.textContent = "Resolve offline blockers before local approval.";
        return;
      }
      rememberUndo();
    ...
    function requestBulkUpdate(status, decision, label) {
      const visibleRows = filteredRows();
      if (!visibleRows.length) {
      ...
      if (!targetRows.length) {
        statusLine.textContent = "No visible rows are eligible for local approval.";
        return;
      }
      ...
      confirmCopy.textContent = label + " " + targetRows.length + " visible rows? "
    ...
    function updateProgress(visibleRows) {
      ...
      reviewProgress.textContent = `${confirmed} approved locally, ${needsFix} need fix, ${blocked} blocked locally`;
    }
    ...
    document.getElementById("bulk-ready").addEventListener("click", () => requestBulkUpdate("confirmed_ready", "bulk_approved_for_dry_run_export", "Approve"));
    document.getElementById("bulk-fix").addEventListener("click", () => requestBulkUpdate("needs_fix", "bulk_requires_fix", "Mark for fixes"));
    document.getElementById("bulk-blocked").addEventListener("click", () => requestBulkUpdate("blocked", "bulk_blocked_from_export", "Block"));
