...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .eyebrow {{ color:var(--brand); font-size:12px; font-weight:700; letter-spacing:.1em; text-transform:uppercase; }}
    ...
    .flow b {{ display:block; color:var(--brand); font:700 10px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace; }}
    ...
    .boundary {{ margin:20px 0 0; padding:14px 16px; border-left:3px solid var(--brand); background:var(--brand-soft); color:var(--body); }}
    .links {{ display:flex; flex-wrap:wrap; gap:14px; margin-top:20px; }}
    .links a {{ color:var(--brand); font-weight:700; }}
    .creator-cta {{ display:flex; align-items:center; justify-content:space-between; gap:24px; margin-top:30px; padding:20px 22px; color:#fff; background:var(--brand); }}
    .creator-cta p {{ margin:0; color:rgba(255,255,255,.88); }} .creator-cta strong {{ display:block; color:#fff; font:700 22px/1.2 Inter,ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }}
    .creator-cta a {{ flex:0 0 auto; min-height:44px; display:inline-flex; align-items:center; padding:9px 12px; border:1px solid #fff; border-radius:8px; color:var(--brand); background:#fff; text-decoration:none; font-weight:750; }}
