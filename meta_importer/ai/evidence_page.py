...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    header a {{ color:var(--forest); text-decoration:none; font-weight:700; }} header .linkedin {{ font-size:12px; text-decoration:underline; text-underline-offset:3px; }}
    ...
    .creator-cta {{ display:flex; align-items:center; justify-content:space-between; gap:24px; margin-top:30px; padding:20px 22px; color:#fff; background:var(--forest); }}
    .creator-cta p {{ margin:0; color:rgba(255,255,255,.78); }} .creator-cta strong {{ display:block; color:#fff; font:500 22px/1.2 "Iowan Old Style",Charter,Georgia,serif; }}
    .creator-cta a {{ flex:0 0 auto; padding:9px 12px; border:1px solid #fff; border-radius:4px; color:var(--forest); background:#fff; text-decoration:none; font-weight:750; }}
