...
<head>
  ...
  <style>
    :root {{ --canvas:#f4f1ea;--surface:#fffdf8;--soft:#f0ede6;--ink:#1d1f1c;--body:#454943;--muted:#666b64;--line:#d7d8d2;--strong:#b8bbb4;--brand:#b83b1f;--brand-hover:#972d18;--brand-soft:#f4dcd4;--success:#176143;--success-soft:#e3eee8;--red:#9e342b;--red-soft:#f3e1dd;--font-serif:"Iowan Old Style",Charter,Georgia,serif;--font-mono:ui-monospace,SFMono-Regular,Menlo,monospace;}}
    *{{box-sizing:border-box}} body{{margin:0;color:var(--ink);background:var(--canvas);font:400 14px/1.5 Inter,ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}} a{{color:var(--brand)}}
    ...
    main{{width:min(1120px,calc(100% - 32px));margin:auto;padding:46px 0 70px}} .eyebrow{{color:var(--brand);font-size:11px;font-weight:750;letter-spacing:.1em;text-transform:uppercase}} h1{{max-width:860px;margin:10px 0 14px;font:500 clamp(40px,6vw,70px)/.98 var(--font-serif);letter-spacing:-.045em}} .lead{{max-width:760px;margin:0;color:var(--body);font-size:18px}}
    ...
    .boundary{{margin-top:24px;padding:14px 16px;border-left:3px solid var(--brand);background:var(--brand-soft);color:var(--body)}} .links{{display:flex;flex-wrap:wrap;gap:14px;margin-top:22px;font-weight:700}} .creator-cta{{display:flex;align-items:center;justify-content:space-between;gap:24px;margin-top:30px;padding:20px 22px;color:#fff;background:var(--brand)}} .creator-cta p{{margin:0;color:rgba(255,255,255,.88)}} .creator-cta strong{{display:block;color:#fff;font:700 22px/1.2 Inter,ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}} .creator-cta a{{flex:0 0 auto;min-height:44px;display:inline-flex;align-items:center;padding:9px 12px;border:1px solid #fff;border-radius:8px;color:var(--brand);background:#fff;text-decoration:none;font-weight:750}}
...
<body>
  <header><nav aria-label="Product navigation"><a href="index.html">&larr; Creative Launch Workspace</a><a class="product-link" href="workspace.html?guided=1">Review a sample batch</a></nav></header>
  <main>
    ...
    <aside class="creator-cta"><p><strong>Continue in the product</strong>Return to the overview or review the full sample batch.</p><a href="index.html">Back to product</a></aside>
  </main>
