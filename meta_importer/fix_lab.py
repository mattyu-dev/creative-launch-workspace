...
<head>
  ...
  <meta property="og:title" content="Fix &amp; Revalidate Lab · Launch Control">
  ...
  <title>Fix &amp; Revalidate Lab · Launch Control</title>
  <style>
    ...
    *{{box-sizing:border-box}} body{{margin:0;color:var(--ink);background:var(--canvas);font:400 14px/1.5 "Mona Sans",ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}} a{{color:var(--brand)}}
    ...
    .boundary{{margin-top:24px;padding:14px 16px;border-left:3px solid var(--accent);background:var(--brand-soft);color:var(--body)}} .links{{display:flex;flex-wrap:wrap;gap:14px;margin-top:22px;font-weight:700}} .continue-cta{{display:flex;align-items:center;justify-content:space-between;gap:24px;margin-top:30px;padding:20px 22px;border-radius:14px;color:#fff;background:var(--brand)}} .continue-cta p{{margin:0;color:rgba(255,255,255,.82)}} .continue-cta strong{{display:block;color:#fff;font-size:22px;font-weight:760;line-height:1.2}} .continue-cta a{{flex:0 0 auto;min-height:44px;display:inline-flex;align-items:center;padding:9px 12px;border:1px solid var(--action);border-radius:8px;color:var(--ink);background:var(--action);text-decoration:none;font-weight:750}}
    @media(max-width:760px){{main{{padding-top:30px}}.lab{{grid-template-columns:1fr}}.result{{border-left:0;border-top:1px solid var(--line)}}.editor,.result{{padding:20px}}.continue-cta{{align-items:flex-start;flex-direction:column}}}}
  </style>
</head>
<body>
  ...
  <main>
    <div class="eyebrow">Deterministic replay</div><h1>Fix a blocked row. Revalidate it.</h1>
    ...
    <aside class="continue-cta"><p><strong>Continue in the product</strong>Open the review queue or go back to the overview.</p><a href="workspace.html?guided=1">Open the workspace</a><a href="index.html">Back to the overview</a></aside>
  </main>
