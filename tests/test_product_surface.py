...
class ProductSurfaceTests(unittest.TestCase):
    def test_homepage_sells_one_product_with_inspectable_boundaries(self) -> None:
        ...
        self.assertIn("From launch sheet", html)
        self.assertIn("Launch sheet", html)
        self.assertNotIn("creative manifest", html.lower())
        self.assertIn("One bad row costs more than the review.", html)
        self.assertIn("You upload the reviewed plan through your normal Ads Manager process.", html)
        self.assertIn("Plans, not publishes.", html)
        self.assertIn("remaining in the workspace", html)
        self.assertIn('id="receipt-empty"', html)
        self.assertIn("No decision recorded yet.", html)
        self.assertIn('href="https://github.com/mattyu-dev/creative-launch-workspace/discussions/24"', html)
        self.assertNotIn("Needs fix", html)
        self.assertNotIn("guided demo", html)
        self.assertNotIn("typed correction", html)
        self.assertNotIn("browser-local", html.lower())
        self.assertIn("Detect, route, decide", html)
        ...
        self.assertIn("Same input, same verdicts, every run.", html)
    ...
    def test_secondary_evidence_page_converts_back_to_mathieu(self) -> None:
        ...
        self.assertIn("Back to the overview", rendered)
